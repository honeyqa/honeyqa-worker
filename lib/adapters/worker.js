var Adapter;
var proguard = require('./proguard.js');
...
(function(S) {
  ...
  P._init = function(callback) {
    ...
    if (!this.hub.options.channel) {
      this.hub.connection.exchange('amq.fanout', {
        passive: true
      }, handle);
    } else {
      this.hub.connection.exchange(this.hub.options.channel, {
        type: 'topic',
        durable: true
      }, handle);
    }
  ...
  P._openCallback = function(exchange) {
    ...
    var queueSettings = {
      ack: self.hub.options.ack !== undefined ? self.hub.options.ack : true,
      prefetchCount: self.hub.options.prefetchCount ? self.hub.options.prefetchCount : 1
    };

    var q = this.hub.connection.queue(this.hub.options.queueName || 'workerQueue', {
      durable: true,
      autoDelete: false
    }, function(queue) {

      queue.subscribe(queueSettings, function(msg) {
        var proguardResult = proguard.getProguardResult();
        self.hub.send(proguardResult.toString());
      });
